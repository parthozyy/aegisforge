...
- attempt process-group/direct-child termination and bounded best-effort direct-child reaping when the deadline or either stream limit is exceeded, reporting cleanup failures without hiding the primary error;
- report timeout, output-limit, reader, termination, and bounded-reap failures as `NativeCommandError::Io`;
...
Replace `Command::output()` with `spawn()`, two pipe-reader threads, elapsed-time deadline supervision, and explicit bounded termination/reap polling. This keeps the existing architecture and dependencies while bounding both time and memory.
...
- `CommandLimits` holds the deadline and per-stream byte cap;
...
- the parent loop polls reader outcomes, reader-handle completion, and elapsed time every 10 milliseconds, polling child state only after both stream EOFs;
...
- cleanup attempts child/process-group termination as supported, uses bounded best-effort polling to reap the direct child, reports if it remains unreaped, and gives readers a fixed close grace period before returning;
...
On Unix, the child starts in its own process group using the safe `CommandExt::process_group(0)` API. Supervision defers `try_wait()` until both reader EOF outcomes exist, so an exited leader whose descendant still holds a pipe remains unreaped and pins the process-group identity. Cleanup sends `SIGTERM` to `-child_pid`, keeps the leader unreaped for the fixed 100-millisecond grace interval, then sends `SIGKILL` to that process group while the identity is still pinned. It independently calls `Child::kill()` for the direct child and uses only bounded `try_wait()` polling to attempt reaping; it reports if the child remains unreaped and never follows an ineffective termination attempt with blocking `wait()`. If the direct child was already reaped after both EOFs, cleanup performs no numeric signaling. `ESRCH` is benign when the group has already exited. This terminates descendants that remain in the child's process group; it is not containment against a program that deliberately escapes the group or session. After forced termination, readers receive a separate fixed 100-millisecond pipe-close grace interval. A handle whose reader still has not completed is detached rather than joined, preserving the runner's return bound. On non-Unix targets, cleanup uses the same direct-child kill plus bounded `try_wait()` polling and nonblocking reader-completion rule. AegisForge's production native caller remains macOS-only and fixed to `/usr/bin/codesign`.
...
5. Poll structured reader events every 10 milliseconds. Poll the child with `try_wait()` only after both streams reach EOF, and track child status separately so an already-reaped child is never signaled or waited twice.
6. Once both readers report complete EOF, poll the direct child until its exit is observed. Only two complete reader outcomes plus child exit before the deadline is success.
7. Treat these as abnormal terminal conditions:
   ...
   - either reader reports an I/O error, its event channel disconnects, or its stream-identified handle finishes without a terminal outcome;
   - the deadline expires before child exit and both reader EOFs;
   - child-state polling fails.
8. On an abnormal condition, attempt to terminate the pinned process group, force escalation after the fixed grace period, independently kill the direct child, and poll for reaping only within a fixed bound. Report an unreaped child as cleanup failure. Wait only a bounded interval for reader completion. Discard all partial output and return `NativeCommandError::Io` with operation context.
...
- Pipe acquisition, read, poll, kill, bounded-reap, or reader-join failures return `Io` with the executable and failed operation.
...
- a sleeping command times out promptly and reaches confirmed reaped state in the controlled test;
...
- state-supervision seams cover reader error/channel-disconnect, a single finished reader without a terminal outcome while its sibling sender remains live, and pipeline-setup cleanup without requiring nondeterministic OS failures;
...
- The direct child is reaped at most once. Supervision observes it only after both pipe EOFs; cleanup skips all signaling when that observation already reaped it.
